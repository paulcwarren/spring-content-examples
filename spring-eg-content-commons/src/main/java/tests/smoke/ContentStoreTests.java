			...
			Context("property path access", () -> {
			    ...
			    Context("given an Entity with content", () -> {
			        ...
			        BeforeEach(() -> {
			            ...
			            claim.setLastName("Smith");
			            claim.setClaimForm(new ClaimForm());
			            claimFormStore.setContent(claim.getClaimForm(), PropertyPath.from("content"), new ByteArrayInputStream("Hello Spring Content World!".getBytes()));
			            claim = claimRepo.save(claim);
			        });
			        ...
			        It("should be able to store new content", () -> {
			            ...
			            InputStream content = null;
			            try {
			                content = claimFormStore.getContent(claim.getClaimForm(), PropertyPath.from("content"));
			                matches = IOUtils.contentEquals(new ByteArrayInputStream("Hello Spring Content World!".getBytes()), content);
			            } catch (IOException e) {
			            } finally {
			                IOUtils.closeQuietly(content);
			        ...
			        Context("when content is updated", () -> {
			            BeforeEach(() ->{
			                claimFormStore.setContent(claim.getClaimForm(), PropertyPath.from("content"), new ByteArrayInputStream("Hello Updated Spring Content World!".getBytes()));
			                claim = claimRepo.save(claim);
			            });
			        ...
			        Context("when content is deleted", () -> {
			            ...
			            It("should have no content", () -> {
			                ...
			                InputStream content = claimFormStore.getContent(deletedClaimForm, PropertyPath.from("content"));
			                Assert.assertThat(content, is(nullValue()));
			                IOUtils.closeQuietly(content);
